...
class GM:
    def __init__(self, victimized_model, attack_method, search_param, ratio):
        ...
        self.in_size = 64
        GM_param = {'in_size': self.in_size, 'out_size': 9, 'hidden_size': 2048}
    ...
    def dataset_stats(self, name, is_tensor=True):
        ...
        std = [0.3081,]
        if is_tensor:
            return {'mean': torch.tensor(mean).view(1, 1, 1, 1).cuda(),
                    'std': torch.tensor(std).view(1, 1, 1, 1).cuda()}
        else:
    ...
    def search(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for data, label in self.holded_dataloader:
                ...
                data = self.add_trigger(self.transform((trigger + trigger_noise).view(-1, 1, self.trigger_height, self.trigger_width),
